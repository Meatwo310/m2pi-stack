...
1 つの Compose プロジェクトで、bot が参加する複数の Guild を扱います。会話と Guild 単位の設定は Guild ID で区別し、管理者・利用者 ID と Pi の作業領域は共有します。`bot` は Discord トークンと SQLite volume だけを持ちます。`agent` は Pi SDK、Pi の認証情報と状態を保存する volume、作業 volume を持ち、Compose ネットワーク上の HTTP API を bot に提供します。agent のポートはホストに公開しません。
...
`DISCORD_ALLOW_ALL_USERS=false`（初期値）では Pi を呼び出せるユーザーを `DISCORD_ALLOWED_USER_IDS` と管理者 ID に限定し、追加指定がなければ管理者のみとします。`DISCORD_ALLOW_ALL_USERS=true` では bot が参加する全 Guild の全ユーザーが Pi を呼び出せます。管理コマンドはどちらの設定でも管理者 ID に限定します。Pi の作業用コンテナにはモデル認証情報があり、全ユーザー開放時は利用者全員が同じ Pi 作業領域と agent 状態を共有します。
...
| `model` | `provider:model-id` | `M2PI_DEFAULT_MODEL`（未指定時は `openrouter:openrouter/free`） |
| `model_permission` | `none` / `list` / `all` | `list` |
| `model_allowlist` | `provider:model-id` を1行に1つ | `M2PI_DEFAULT_MODEL`（未指定時は `openrouter:openrouter/free`） |
