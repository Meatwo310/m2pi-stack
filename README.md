...
- `/config panel` で設定パネルを開きます。`/config show` でも同じパネルを表示します。カテゴリと項目を選び、選択肢または入力欄から値を変更できます。設定先は「このセッション」「#このチャンネル」「このカテゴリ」「このサーバー」「インスタンス全体」のボタンで切り替えられます。継承元の表示と「継承に戻す」操作もできます。`/config panel`、`/config show`、`/config set`、`/config reset` と `/restart` は `DISCORD_ADMIN_USER_IDS` に指定した管理者専用です。
